    ...
    public class Sender
    {
        ...
        public string GetTypeOfTheTask(string hint)
        {
            return hint switch
            {
                ...
                => "math",
                @"Посчитай определитель матрицы. Во входных данных строки разделяются двойным обратным слэшем \\, а столбцы амперсандом &."
                => "determinant",
                @"Найди любой корень полинома, либо передай в качестве ответа ""no roots"", если корней нет." =>
                "polynomial-root",
                @"Расшифруй сообщение, используя подсказки. Все сообщения взяты из Ты-Знаешь-Какого-Текста. Но текст был предварительно подготовлен, поэтому сообщения содержат только латинские буквы в нижнем регистре, цифры, апостроф (') и пробелы. Пробелы не повторяются."
                => "cypher",
                @"Определи, что за фигура задана точками, а затем выбери соответствующий ответ из предложенных" =>
                "shape",
                @"Найди обратную матрицу. Во входыных данных и в ответе строки матрицы разделяются двойным обратынм слэшем \\, а столбцы - амперсандом &. Если матрица необратима, то передай ""unsolvable"" в качестве ответа."
                => "inverse-matrix",
